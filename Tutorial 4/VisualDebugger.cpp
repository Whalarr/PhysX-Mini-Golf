...
namespace VisualDebugger
{
	...
	PxReal delta_time = 1.f/5.f;
	PxReal gForceStrength = 300;

	PxReal swing_Strength = 3000;
	PxReal swing_Hold = 1;
	...
	void CameraInput(int key)
	{
		...
		{
		case 'W':
			camera->MoveForward(delta_time * 4);
			break;
		case 'S':
			camera->MoveBackward(delta_time* 4);
			break;
		case 'A':
			camera->MoveLeft(delta_time * 4);
			break;
		case 'D':
			camera->MoveRight(delta_time * 4);
			break;
		case 'Q':
			camera->MoveUp(delta_time * 4);
			break;
		case 'Z':
			camera->MoveDown(delta_time * 4);
			break;
		default:
			break;
		}
	...
	void ForceInput(int key)
	{
		...
		case 'I': //forward
			...
			scene->Golf_Ball->Get()->isRigidBody()->addTorque(PxVec3(-1, 0, 0)*(swing_Strength * gForceStrength));
			break;
		case 'K': //backward
			//scene->GetSelectedActor()->addForce(PxVec3(0,0,1)*gForceStrength);
			scene->Golf_Ball->Get()->isRigidBody()->addTorque(PxVec3(1, 0, 0)*(swing_Strength * gForceStrength));
		...
		case 'J': //left
			...
			scene->Golf_Ball->Get()->isRigidBody()->addTorque(PxVec3(0, 0, 1)*(swing_Strength * gForceStrength));
			break;
		case 'L': //right
			//scene->GetSelectedActor()->addForce(PxVec3(1,0,0)*gForceStrength);
			scene->Golf_Ball->Get()->isRigidBody()->addTorque(PxVec3(0, 0, -1)*(swing_Strength * gForceStrength));
		...
		case 'U': //up
			...
			scene->Golf_Ball->Get()->isRigidBody()->addForce(PxVec3(0, 1, 0)*gForceStrength);
			break;
		case 'M': //down
			//scene->GetSelectedActor()->addForce(PxVec3(0,-1,0)*gForceStrength);
			scene->Golf_Ball->Get()->isRigidBody()->addForce(PxVec3(0, -1, 0)*gForceStrength);
	...
	void mouseCallback(int button, int state, int x, int y)
	{
		...
		if (button == 0)
		{
			...
			PxVec3 camera_rotation = camera->getDir();
			//scene->Golf_Ball->Get()->isRigidBody()->addForce(PxVec3(0.0, 0.0, -1.0)*(swing_Strength * swing_Hold * 100000));
		}
	}
